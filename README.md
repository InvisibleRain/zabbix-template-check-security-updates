...
## Debian
1. Add apt/crontab to your cron
2. Import the apt/apt-security-updates.yml template
...
1. Copy scripts from dnf/scripts to /opt/bin/zabbix-agent/
2. Run each script manually and verify that /var/run/zabbix/ has new files
3. Copy dnf/zabbix-agent-script@.service and dnf/zabbix-agent-script@.timer to /etc/systemd/system/
...
5. Import the dnf/dnf-security-updates.yml template
